 ...
 *
 * <p>Spring's default is {@code <topic>.DLT}. Two services read {@code bookstore.order.placed} here, so
...
public class KafkaErrorHandlingConfig {
    ...
    public NewTopic orderPlacedDeadLetterTopic(
            ...
            @Value("${app.kafka.groups.notification}") String group) {

        return TopicBuilder.name(deadLetterNameFor(topic, group)).partitions(3).replicas(1).build();
    }

    @Bean
    public NewTopic paymentCompletedDeadLetterTopic(
