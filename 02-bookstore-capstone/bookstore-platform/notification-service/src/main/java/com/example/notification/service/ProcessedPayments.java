...
/**
 * Remembers which orders have already had a receipt sent.
 *
 * <p>A near-copy of {@link ProcessedOrders}, on purpose: see {@link ReceiptSender} for why one shared
 * guard between two units of work would be a bug rather than a saving.
 *
...
public class ProcessedPayments {
    ...
    public boolean firstTimeSeeing(Long orderId) {
        ...
        if (!added) {
            log.info("Order {} has already had a receipt; ignoring redelivery", orderId);
        }
        return added;
    }
}
