...
public class GlobalExceptionHandler {
     ...
     * catalog; order-service learns about it only as a 409 from book-service, translated by
    ...
    public ResponseEntity<ErrorResponse> handleNotAllowed(OrderNotAllowedException ex,
                                                          HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
    ...
    /**
     * book-service could not be reached, or refused this service's credentials.
     *
    ...
    /**
     * book-service could not be reached at all — connection refused, DNS failure, or a timeout.
     *
     * <p>Separate from {@link CatalogUnavailableException} because it arrives by a different route, and
    ...
    public ResponseEntity<ErrorResponse> handleFeignTransport(feign.FeignException ex,
                                                              HttpServletRequest request) {
        log.error("Could not reach book-service on {} {}: {}",
                request.getMethod(), request.getRequestURI(), ex.getMessage());
        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                .body(ErrorResponse.of(503, "Service Unavailable",
                        "The catalog service is temporarily unavailable. Please try again.",
